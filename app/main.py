from PyQt5.QtWidgets import (QApplication, QMainWindow, QVBoxLayout, QPushButton, QLabel, 
                           QFileDialog, QWidget, QMenuBar, QMenu, QTextEdit, QHBoxLayout,
        ...
        if file_path:
            try:
                if file_path.endswith('.npy'):
                    ...
                    # Ensure it's 2D
                    if data.ndim != 2:
                         raise ValueError(f"Expected a 2D array from .npy file, but got shape {data.shape}")
                ...
                self.matrix_display.setText(f"Matrix Shape: {self.matrix_data.shape}\n\n{str(self.matrix_data)}")

                self.status_label.setText(f"File imported successfully: {file_path}")
                self.statusBar.showMessage(f"Matrix loaded: {self.matrix_data.shape[0]}x{self.matrix_data.shape[1]}")
